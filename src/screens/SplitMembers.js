...
import { Container, CheckBox, Header, Title, Content, ListItem, Button, Left, Right, Body, Icon, Text, View, Footer, FooterTab, Spinner, ImageBackground } from 'native-base';
...
export default class SplitMembers extends Component {
    ...
    componentDidMount = () => {
        auth.onAuthStateChanged((user) => {
            if (user) {
                ...
                db.collection('AddMembers').where("userId", "==", user.uid).onSnapshot((querySnapshot) => {
                    let data = querySnapshot.docs
                    data.forEach((obj) => {
                        let member = obj.data()
                        member.checked = false
                        //SetState
                        this.setState({ members: [...this.state.members, member], loading: false })
                    })
                });
            } else {
    ...
    amountSplit = () => {
        ...
        if (checkedMembers.length !== 0) {
            ...
            Communications.web(`https://venmo.com/?txn=charge&amount=${amountPerPerson}&note=for+dinner&recipients=${memberVenmoId}`)
            ...
            <Container>
                ...
                <Content padder>
                    {this.state.loading == true
                        ...
                        : (<ScrollView>
                            <Text style={{ color: 'teal', textAlign: "center" }}>Please select the members, you want to share the amount.</Text>
                            {this.state.members.length !== 0
                                ? this.state.members.map((member, i) => {
                                    return (
                                        <ListItem key={i}>
                                            <CheckBox color='teal' checked={member.checked} onPress={() => this.checkMember(member)} />
                                            <Body style={{ marginStart: 15 }}>
                                                <View style={{ flexDirection: 'row' }}>
                                                    <Text>Name: </Text><Text style={{ fontSize: 18, color: 'green' }}>{member.memberName}</Text>
                                                </View>
                                                <View style={{ flexDirection: 'row' }}>
                                                    <Text>Venmo ID: </Text><Text style={{ fontSize: 18, color: 'green' }}>{member.venmoId}</Text>
                                                </View>
                                            </Body>
                                        </ListItem>
                                    )
                                ...
                                : <View>
                                    <Text style={{ color: 'red', textAlign: "center", fontSize: 24, marginTop: 30 }}>NO MEMBERS FOUND</Text>
                                </View>
                            }
                        </ScrollView>)
                    }
